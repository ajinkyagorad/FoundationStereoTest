...
def undistort_image(img, K, dist):
    ...
    # You can crop the image to the ROI or keep the full size (using padding if needed)
    x, y, w, h = roi
    undistorted = undistorted[y:y+h, x:x+w]
    
    # If you want to keep the full image size (even after cropping), you can pad it back
    undistorted = cv2.copyMakeBorder(undistorted, 0, h - undistorted.shape[0], 0, w - undistorted.shape[1], cv2.BORDER_CONSTANT, value=(0, 0, 0))
...
if __name__ == "__main__":
    ...
    while True:
        # Always grab the latest frames, discard any old frames
        cap_left.grab()  # Grabs the next frame from the left camera without decoding
        cap_right.grab()  # Grabs the next frame from the right camera without decoding

        curr_sec = int(time.time() - start_time)
        if curr_sec - prev_sec >= 0:  # Check if 1 second has passed
            prev_sec = curr_sec

            # Retrieve the latest frames when you're ready to process them
            ret_left, img_left = cap_left.retrieve()
            ret_right, img_right = cap_right.retrieve()
            ...
            #cv2.imshow("Left Undistorted Image", img_left)
            #cv2.imshow("Right Undistorted Image", img_right)
            ...
            # Fix color channels (Open3D expects RGB, OpenCV gives BGR)
            img_left_resized_rgb = cv2.cvtColor(img_left_resized, cv2.COLOR_BGR2RGB)

            # Filter valid points and corresponding colors
            xyz_map_filtered = xyz_map[valid_mask]
            colors_filtered = img_left_resized_rgb.reshape(-1, 3)[valid_mask.reshape(-1)] / 255.0
